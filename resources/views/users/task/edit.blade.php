        ...
        <form action="{{ route('tasks.update', $task->id) }}" method="POST" id="form">
            ...
            <div class="block block-rounded">
                <div class="block-header block-header-default">
                    <h3 class="block-title">Form Edit Task</h3>
                </div>
                @if(session('error'))
                        ...
                        <div class="col-lg-10 col-xl-10">
                            ...
                            <div class="mb-4">
                                ...
                                <input type="text" class="form-control @error('name') is-invalid @enderror"
                                    ...
                                    value="{{ old('name', $task->name) }}" readonly>
                                @error('name')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="mb-4">
                                ...
                                <textarea
                                    ...
                                    id="description" name="description" rows="4" placeholder="Description" readonly>{{ old('description', $task->description) }}</textarea>
                                @error('description')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            <div class="mb-4">
                                <label class="form-label" for="user[]">Member on this task</label>
                                <select class="js-select2 form-control @error('user') is-invalid @enderror"
                                    id="user_select" style="width: 100%;" data-placeholder="Choose at least two.." multiple disabled>
                                    @foreach ($users as $user)
                                        <option value="{{ $user->id }}" {{ $task->user->contains($user->id) ? 'selected' : '' }}>
                                            {{ $user->name }}
                                        </option>
                                    @endforeach
                                </select>
